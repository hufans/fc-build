...
pub async fn run_command_hook(
    ...
    let mut child = match cmd
        ...
        // SECURITY: extra_env is applied before the FC_* identity vars so a hook cannot spoof them.
        .envs(&spec.extra_env)
        ...
        .env("FC_WORKSPACE_ROOT", env_root.as_ref())
...
///   hooks),
/// * the fc process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn powershell_rewrite_cases() {
        ...
        let cases = [
            ...
            (
                "$CLAUDE_PROJECT_DIR/$FC_HOOK_NAME.ps1",
                r#"& "$env:CLAUDE_PROJECT_DIR/$env:FC_HOOK_NAME.ps1""#,
            ),
            (

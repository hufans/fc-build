...
pub async fn run_http_hook(
    ...
    for (k, v) in [
        ("FC_HOOK_EVENT", envelope.hook_event_name.to_string()),
        ("FC_HOOK_NAME", spec.name.clone()),
        ("FC_SESSION_ID", ctx.session_id.to_string()),
        ("FC_WORKSPACE_ROOT", ctx.workspace_root.to_string()),

...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.fc/sandbox.toml
    let global_path = grok_home().join(SANDBOX_CONFIG_FILENAME);
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: prefer `.fc/sandbox.toml`, fall back to legacy `.grok`.
    for rel in [
        format!(".fc/{SANDBOX_CONFIG_FILENAME}"),
        format!(".grok/{SANDBOX_CONFIG_FILENAME}"),
    ] {
        let project_path = workspace.join(&rel);
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    ...
    .into_iter()
    .find_map(|rel| load_config_file(&workspace.join(rel)))
    .unwrap_or_default();

...
fn release_repo() -> String {
    std::env::var("FC_RELEASE_REPO").unwrap_or_else(|_| DEFAULT_REPO.to_string())
}

fn release_tag() -> String {
    std::env::var("FC_RELEASE_TAG").unwrap_or_else(|_| DEFAULT_TAG.to_string())
}

/// True when this process should use the fc update channel.
pub fn running_as_kiro() -> bool {
    std::env::current_exe()
        .ok()
        .and_then(|p| {
            p.file_name()
                .map(|n| n.to_string_lossy().eq_ignore_ascii_case("fc"))
        })
        .unwrap_or(false)
}

/// Map host platform to CI artifact name (`fc-darwin-arm64`, etc.).
pub fn artifact_name() -> Result<&'static str> {
    let os = std::env::consts::OS;
    let arch = std::env::consts::ARCH;
    match (os, arch) {
        ("macos", "aarch64") => Ok("fc-darwin-arm64"),
        ("macos", "x86_64") => bail!(
            "Intel Mac (x86_64) binaries are not published on the continuous channel.\n\
             Build from source:\n  \
             cargo build -p xai-grok-pager-bin --release --bin fc"
        ),
        ("linux", "x86_64") => Ok("fc-linux-x86_64"),
        ("linux", "aarch64") => bail!("linux arm64 binaries are not published yet"),
        _ => bail!("unsupported platform for fc update: {os}-{arch}"),
    }
}
...
/// Parse `fc 0.2.112 (47c2f2f)` → `0.2.112+47c2f2f` (semver + build meta for equality).
...
/// Install destinations: current exe first, then common fc install paths.
fn install_destinations() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs_home() {
        ...
        // Always ensure ~/.local/bin/fc is a target so users can leave
        // ~/.grok/bin without losing updates.
        let local = home.join(".local/bin/fc");
...
/// Download continuous artifact and atomically replace fc install paths.
pub async fn install(_pinned: Option<&str>) -> Result<()> {
    let artifact = artifact_name()?;
    let url = asset_url(artifact);
    let dests = install_destinations();
    if dests.is_empty() {
        bail!("could not resolve install path for fc");
    }

    eprintln!(
        "  Downloading fc ({artifact}) from GitHub Releases ({}/{})...",
        release_repo(),
        release_tag()
    );

    let tmp_dir = std::env::temp_dir().join(format!("fc-update-{}", std::process::id()));
    tokio::fs::create_dir_all(&tmp_dir).await?;
    let tmp_bin = tmp_dir.join("fc");
    ...
    eprintln!("  downloaded: fc {new_ver}");
    ...
    // Remember installer type for future `fc update` / auto-update.
    let _ = xai_grok_shell::util::config::update_config(|st| {
        st.cli.installer = Some("fc".to_string());
    })
    .await;

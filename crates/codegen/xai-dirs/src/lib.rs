...
//! fc-home (`$FC_HOME` / `$KIRO_HOME` / `$GROK_HOME`, else `<home>/.fc`).
...
/// Where a resolved home came from, so "why did fc pick this
...
pub enum GrokHomeSource {
    /// A non-empty `$FC_HOME`, `$KIRO_HOME`, or `$GROK_HOME` override.
    EnvOverride,
    /// `<home>/.fc` derived from the home directory.
    HomeDefault,
}
...
/// resolves `~/.fc` from the profile variable — mixing the two sources puts
/// the fc directory and other home-anchored dot directories in different
...
fn resolve_grok_home_from(
    ...
    for env in [fc_home, kiro_home, grok_home_env] {
        if let Some(v) = env.filter(|e| !e.is_empty()) {
            return Some((PathBuf::from(v), GrokHomeSource::EnvOverride));
        }
    }
...
mod tests {
    ...
    fn fc_env_wins_over_os_home() {
        ...
        assert_eq!(
            resolved,
            Some((PathBuf::from("/custom/fc"), GrokHomeSource::EnvOverride))
        );
    }
    ...
    fn kiro_env_wins_when_fc_unset() {
        ...
        assert_eq!(
            resolved,
            Some((PathBuf::from("/custom/kiro"), GrokHomeSource::EnvOverride))
        );
    }
